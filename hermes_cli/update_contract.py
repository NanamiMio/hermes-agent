...
class UpdateRefusal:
    ...
    code: str              # image-marker | image-marker-invalid | docker | nix | apt | brew-tap
...
def evaluate_update_admission(project_root: Path) -> Optional[UpdateRefusal]:
    ...
    try:
        ...
        if method == "brew-tap":
            return _refusal("brew-tap", method, lambda command: (
                "✗ This is a Homebrew (tap) install; in-place update is disabled.\n"
                f"  Update with:\n    {command}"
            ))
